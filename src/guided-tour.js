...
export class GuidedTour {
  constructor(demoKey) {
    const fallback = precomputed['workshop']?.tour || { inputSteps: [], outputSteps: [] }
  ...
  createHighlight(step) {
    const targetContainer = this.currentPhase === 'input' ? this.editor : this.output
    ...
    const tourSegment = targetContainer.querySelector(`[data-tour-id="${step.id}"]`)
    if (tourSegment) {
  ...
  positionTooltip(tooltip, step) {
    const container = this.currentPhase === 'input' ? this.editor : this.output
    const anchor = container?.querySelector(`[data-tour-id="${step.id}"]`) || container
    if (!container || !anchor) {
    ...
    switch (chosen.name) {
      case 'bottom':
        ...
        left = (anchorRect.left - containerRect.left) + container.scrollLeft + (anchorRect.width - tipRect.width) / 2
        break
      case 'top':
        top = (anchorRect.top - containerRect.top) + container.scrollTop - tipRect.height - gap
        left = (anchorRect.left - containerRect.left) + container.scrollLeft + (anchorRect.width - tipRect.width) / 2
        break
      case 'right':
        top = (anchorRect.top - containerRect.top) + container.scrollTop + (anchorRect.height - tipRect.height) / 2
        left = (anchorRect.right - containerRect.left) + container.scrollLeft + gap
        break
      case 'left':
      default:
        top = (anchorRect.top - containerRect.top) + container.scrollTop + (anchorRect.height - tipRect.height) / 2
        left = (anchorRect.left - containerRect.left) + container.scrollLeft - tipRect.width - gap
        break
    }
    ...
    const maxLeft = container.scrollLeft + (containerRect.width - tipRect.width)
  ...
  calculateOptimalPosition(anchorRect, tipRect) {
    ...
    const positions = [
      { placement: 'bottom', needsSpace: tipRect.height + gap, available: spaces.bottom },
      { placement: 'top', needsSpace: tipRect.height + gap, available: spaces.top },
      { placement: 'right', needsSpace: tipRect.width + gap, available: spaces.right },
      { placement: 'left', needsSpace: tipRect.width + gap, available: spaces.left }
    ]

    // Find best placement (preferring those that fit completely)
    const fitting = positions.filter(p => p.available >= p.needsSpace)
    const chosen = fitting.length > 0 
    ...
    top = Math.max(viewport.top, Math.min(viewport.bottom - tipRect.height, top))
    left = Math.max(viewport.left, Math.min(viewport.right - tipRect.width, left))

    // Final overlap check - if still overlapping, use fallback position
    const overlaps = !(left + tipRect.width <= anchorRect.left || left >= anchorRect.right || 
                      top + tipRect.height <= anchorRect.top || top >= anchorRect.bottom)
  ...
  getFallbackPosition(anchorRect, tipRect, viewport) {
    ...
    const top = Math.max(viewport.top, Math.min(viewport.bottom - tipRect.height, centerY - tipRect.height / 2))
  ...
  clearHighlights() {
    this.highlightElements.forEach(element => {
      if (element && element.classList) {
        ...
        element.classList.remove('tour-line-highlight', 'tour-pattern-highlight', 'tour-active')
  ...
  async transitionToOutput() {
    ...
    const hasOutput = output && output.children.length > 0 && 
                     !output.textContent.includes('Click Run to see')
  ...
  async autoRunSynthesis() {
    // Import the runDemo function dynamically to avoid circular imports
    const { runDemo } = await import('./highlight-setup.js')
    
    return new Promise((resolve, reject) => {
      try {
        // Set up a listener for synthesis completion
  ...
  mountControls() {
    ...
    el.className = 'tour-controls'
    el.innerHTML = `
      <button type="button" class="tour-prev-btn btn" aria-label="Back">Back</button>
      <button type="button" class="tour-next-btn btn bg-accent" aria-label="Next">Next</button>
      <button type="button" class="tour-restart-btn btn" aria-label="Restart">Restart</button>
      <button type="button" class="tour-skip-btn btn" aria-label="Stop">Stop</button>
      <span class="tour-status" aria-live="polite"></span>
    `
    host.appendChild(el)
    this.controlsEl = el

    el.querySelector('.tour-prev-btn')?.addEventListener('click', this.previousStep)
    el.querySelector('.tour-next-btn')?.addEventListener('click', this.nextStep)
    el.querySelector('.tour-restart-btn')?.addEventListener('click', this.restartTour.bind(this))
    el.querySelector('.tour-skip-btn')?.addEventListener('click', this.skipTour)
  ...
  updateControls() {
    ...
    const steps = this.getCurrentSteps()
    const isFirst = this.currentStepIndex === 0 && this.currentPhase === 'input'
    const isLast = this.currentStepIndex === steps.length - 1 && this.currentPhase === 'output'

    const prevBtn = this.controlsEl.querySelector('.tour-prev-btn')
    const nextBtn = this.controlsEl.querySelector('.tour-next-btn')
    const status = this.controlsEl.querySelector('.tour-status')
    ...
    if (nextBtn) nextBtn.setAttribute('aria-label', isLast ? 'Finish' : 'Next')
    if (nextBtn) nextBtn.textContent = isLast ? 'Finish' : 'Next'
    if (status) status.textContent = `${this.currentPhase === 'input' ? 'Input' : 'Output'} ${this.currentStepIndex + 1}/${steps.length}`
  }

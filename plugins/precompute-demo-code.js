...
function buildLineHtml(lines, language) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const highlighted = content.trim() === ''
      ? '<span class="empty-line">&nbsp;</span>'
      : `<span class="line-content">${highlightLine(content, language)}</span>`
    htmlLines.push(`<div class="code-line" data-line-number="${lineNumber}">${highlighted}</div>`)
  }
  return htmlLines
}
...
export default function precomputeDemoCode() {
  ...
  async function buildModuleCode() {
    ...
    for (const key of Object.keys(demos)) {
      ...
      const typescript = precomputeOne({ code: tsCode, language: 'typescript', steps: tour?.inputSteps || [] })
      const terraform = precomputeOne({ code: tfCode, language: 'hcl', steps: tour?.outputSteps || [] })
  ...
  return {
    ...
    handleHotUpdate(ctx) {
      ...
      if (
        ...
        Object.values(demos).some(d => d.typescriptPath === changed || d.terraformPath === changed)
      ) {

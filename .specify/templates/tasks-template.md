...
* **Epic**: website-[epic-id] → represents the whole feature
...
# Add a new task
bd create "Implement OAuth redirect handler" -t task --parent [website-auth-feature] --label spec:[feature-name] --label component:backend-services

# Update notes on a task
bd update website-xyz123 --notes "Re-use helper functions from auth module"

# Add a comment to an issue based on research or findings during implementation
bd comments add website-xyz123 "Additional research identified bcrypt as best hashing algo"
```

# Mark task as completed with context

```bash
bd close website-xyz123 --reason "Completed with bcrypt, 12 rounds, <100ms"
```

or use `update` to set status and additional fields.

```bash
bd update website-xyz123 --status closed --notes "Completed with bcrypt, 12 rounds, <100ms"
```
